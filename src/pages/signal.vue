		...
		<f7-page>
		    ...
		    <f7-subnavbar>
		        <f7-segmented class="segmented-raised">
		            <f7-link class="mygreenbutton" tab-link href="#tab-1" tab-link-active>По типу</f7-link>
		            <f7-link class="mygreenbutton" tab-link href="#tab-2">Все</f7-link>
                ...
                <li v-for="listall in $root.tasks" :key="listall.id_task" class="swipeout">
                  <a :href="'/analize/' + listall.id_task + '/'" class="item-link item-content">
                    ...
                    <div class="swipeout-actions-right swipeout-close">
                      <f7-link class="bg-green" @click="openActionsPopover(listall.id_task)">Взять в работу</f7-link>
                    </div>
                  </a>
                </li>
              </ul>
	        ...
	        self.actionsToPopover = app.actions.create({
	          buttons: [
	            ...
	            },
	            {
	              ...
	              bold: true,
			      onClick: function () {
			        app.dialog.alert('Button1 clicked '+id)
			      }
	            },
	            {
	              text: 'Нормальный приоритет',
	             // onClick: this.TakeInWork(id)
	            },
	            {
